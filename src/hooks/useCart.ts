...
export const useCart = () => {
  ...
  function addToCart(guitarItem: Guitar) {
    ...
    if (itemExist >= 0) {
      ...
      const newItem: CartItem = { ...guitarItem, quantity: 1 };
      setCart([...cart, newItem]);
    }
  }

  // Function Remove
  function removeFromCart(idGuitarItem: Guitar["id"]) {
    setCart((prevCart) => prevCart.filter((guitar) => guitar.id !== idGuitarItem));
  }

  // Funcion INCREMENTAR
  function increaseQuantity(idGuitarItem: Guitar["id"]) {
  ...
  // Funcion DECREMENTAR
  function decreaseQuantity(idGuitarItem: Guitar["id"]) {
    const updatedCart = cart.map((item) => {
